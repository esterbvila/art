...
export default function CollectionPage({ collection, artworks }) {
	...
	const minPrice =
		artworks.length > 0 ? Math.min(...artworks.map((a) => a.price)) : null;
	const heroImage =
	...
	return (
		<>
			<Head>
				...
				<meta
					property="og:title"
					content={`${collection.name} — Ester Batllori`}
				/>
				<meta
				...
				<meta
					property="og:url"
					content={`https://esteriicreates.com/collections/${collection.slug}`}
				/>
				...
				<link
					rel="canonical"
					href={`https://esteriicreates.com/collections/${collection.slug}`}
				/>
				<script
...
export async function getServerSideProps({ params }) {
	...
	const { data: collection, error: colError } = await supabase
		.from("collections")
		.select(
			"id, slug, name, description_collection, tagline, cover_image_url, hero_image, price",
		)
		.eq("slug", slug)
		.single();
	...
	const cover_image_url = collection.cover_image_url
		? ((await resolveFirstImage(collection.cover_image_url)) ??
			collection.cover_image_url)
		: null;

	const hero_image = collection.hero_image
		? ((await resolveFirstImage(collection.hero_image)) ??
			collection.hero_image)
		: null;
