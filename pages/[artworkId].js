		...
		<>
			<Head>
				...
				<meta
					...
					content={
						artwork.description ||
						`${artwork.title} — Original abstract painting by Ester Batllori.`
					}
				/>
				<meta
					property="og:title"
					content={`${artwork.title} — Ester Batllori`}
				/>
				<meta
				...
				<meta
					property="og:url"
					content={`https://esteriicreates.com/${artwork.id}`}
				/>
				{artwork.image_url && (
				...
				<link
					rel="canonical"
					href={`https://esteriicreates.com/${artwork.id}`}
				/>
				<script
					...
					<div className="md:w-1/2 lg:w-[55%] md:max-w-[563px] lg:max-w-none">
						...
						<div className="hidden md:flex lg:hidden flex-col gap-[6px]">
							{(artwork.images ?? []).map((src, i) => (
								<div
									key={i}
									className="relative w-full aspect-[3/4] overflow-hidden group cursor-zoom-in"
						...
						<div className="hidden lg:flex flex-col gap-[6px]">
							{artwork.images?.[0] && (
								<div
									className="relative w-full max-w-[640px] 2xl:max-w-[670px] mx-auto aspect-[3/4] overflow-hidden group cursor-zoom-in"
								...
								.map((pair, rowIdx) => (
									<div key={rowIdx} className="flex gap-[6px]">
										{pair.map((src, colIdx) => (
											<div
												key={colIdx}
												className={`relative aspect-[387/500] overflow-hidden group cursor-zoom-in ${pair.length === 1 ? "w-1/2" : "flex-1"}`}
												onClick={() => setLightboxSrc(src)}
											>
												<Image
													src={src}
						...
						<div className="flex flex-col gap-[14px]">
							{details.map(({ label, value, accent }) => (
								<div key={label} className="flex justify-between items-center">
									<span className="text-text-tertiary font-sans text-[13px]">
										{label}
									</span>
									<span
										className={`font-sans text-[13px] ${accent ? "text-accent" : "text-text-primary"}`}
									>
										{value}
									</span>
								</div>
						...
						<div className="py-10 md:py-[56px] lg:px-[56px] w-full">
							...
							<div className="flex lg:hidden gap-3 overflow-x-auto pb-2 snap-x snap-mandatory scrollbar-hide scroll-pl-5 pl-5 pr-5">
								{related.map((a) => (
									<div
										key={a.id}
										className="flex-shrink-0 w-[50vw] sm:w-[30vw] snap-start"
									>
										<ArtworkCard
											artwork={a}
											imageHeight="h-[230px]"
...
export async function getServerSideProps({ params }) {
	...
	const { data: artwork, error } = await supabase
		.from("artworks")
		.select(
			"*, collections(id, slug, name, description, process, medium, year, price)",
		)
		.eq("id", artworkId)
		.single();
	...
	return {
		props: { artwork: artworkData, collection: collection ?? null, related },
	};
}
