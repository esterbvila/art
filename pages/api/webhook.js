...
export default async function handler(req, res) {
	...
	if (event.type === "checkout.session.completed") {
		...
		const rawIds =
			session.metadata?.artworkIds ?? session.metadata?.artworkId ?? "";
		...
		const { error: firstOrderError } = await adminSupabase
			.from("orders")
			.insert({
				...sharedFields,
				artwork_id: firstId,
		...
		if (firstOrderError) {
			console.error(
				`Failed to insert order for ${firstId}:`,
				firstOrderError.message,
			);
		...
		for (const artworkId of artworkIds.slice(1)) {
			...
			if (orderError) {
				console.error(
					`Failed to insert order for ${artworkId}:`,
					orderError.message,
				);
			}
		}
		...
		if (customerEmail) {
			const artworks = artworkIds.map(
				(id) => artworkMap[id] ?? { title: "your painting", price: 0 },
			);
