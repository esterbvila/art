...
export default function FeaturedPainting({ artwork }) {
  ...
  return (
    <section className="flex flex-col md:flex-row bg-bg-deep w-full md:h-[560px]">
      ...
      {/* ── Content panel ─────────────────────────────────────────────── */}
      <div
        className="flex flex-col justify-center gap-5 px-5 py-8 md:px-[56px] md:py-[64px] md:w-[500px] md:flex-shrink-0"
      >
        {/* Label */}
        ...
        <h2
          ...
          style={{ fontSize: 'clamp(36px, 3.3vw, 48px)', letterSpacing: '-1.5px' }}
        >
          {artwork.title}
        </h2>
