...
export default function ArtworkCard({ artwork, imageHeight }) {
  ...
  return (
    <Link href={`/${id}`} className="group flex w-full cursor-pointer flex-col" aria-label={`View ${title}`}>
      <div className={`relative max-h-[410px] w-full overflow-hidden ${imageHeight ?? "aspect-3/4"}`}>
        <Image
          src={image_url}
          alt={title}
          fill
