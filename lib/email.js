...
export async function sendOrderConfirmation({
  ...
  const shippingHtml = shippingLines.map((l) => `<div>${esc(l)}</div>`).join('');
  ...
  const introLine = isSingle
    ? `I have received your order of <strong style="color:#1A1917;">${esc(artworks[0].title)}</strong> and your payment details successfully.`
  ...
  const artworkRows = isSingle
    ? `<tr>
        ...
        <td style="padding:20px 0;font-size:13px;color:#1A1917;text-align:right;">${esc(artworks[0].title)}</td>
       </tr>`
    : artworks.map((a, i) => `
        <tr>
          <td style="padding:${i === 0 ? '20px' : '0'} 0 12px 0;font-size:13px;color:#9C9690;">${esc(a.title)}</td>
          ...
          <tr>
            <td style="padding:40px 0;">
              ...
              <h1 style="margin:0 0 24px 0;font-size:36px;font-weight:400;color:#1A1917;line-height:1;letter-spacing:-1px;">
                Thank you for your order, ${esc(customerName.split(' ')[0])}.
              </h1>
 ...
 */
export async function notifySale({ customerName, customerEmail, artworks, amountCents, phone, shipping, paymentMethod, message }) {
  const from = process.env.RESEND_FROM_EMAIL ?? 'esterii creates <orders@yourdomain.com>';
  const to   = process.env.NOTIFY_EMAIL ?? 'ester.batllori@gmail.com';
  ...
  const artworksList = artworks.map(a => `<p style="margin:4px 0"><strong>${esc(a.title)}</strong> — ${formatPrice(a.price)}</p>`).join('');
  ...
  const { error } = await resend.emails.send({
    ...
    subject: subjectLine,
    html: `
      <h2 style="margin:0 0 16px 0">${esc(subjectLine)}</h2>
      ${artworksList}
      <p><strong>Total:</strong> ${formatPrice(amountCents)}</p>
      ${paymentMethod ? `<p><strong>Payment method:</strong> ${esc(paymentMethod)}</p>` : ''}
      <hr/>
      <p><strong>Buyer:</strong> ${esc(customerName)}</p>
      <p><strong>Email:</strong> ${esc(customerEmail)}</p>
      ${phone ? `<p><strong>Phone:</strong> ${esc(phone)}</p>` : ''}
      ${shippingLines.length > 0 ? `
      <p><strong>Ship to:</strong><br/>${shippingLines.map(esc).join('<br/>')}</p>
      ` : ''}
      ${message ? `<hr/><p><strong>Message from buyer:</strong><br/>${esc(message)}</p>` : ''}
    `.trim(),
  });
