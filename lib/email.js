...
export async function sendOrderConfirmation({
  ...
  const introLine = isSingle
    ? `I have received your order of <strong style="color:#1A1917;">${artworks[0].title}</strong> and your payment details successfully.`
...
export async function notifySale({ customerName, customerEmail, artworks, amountCents, phone, shipping, paymentMethod, message }) {
  ...
  const { error } = await resend.emails.send({
    ...
    html: `
      ...
      ` : ''}
      ${message ? `<hr/><p><strong>Message from buyer:</strong><br/>${message}</p>` : ''}
    `.trim(),
  });
